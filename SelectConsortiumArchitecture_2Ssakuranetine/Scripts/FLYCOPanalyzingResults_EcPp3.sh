...
# call: FLYCOPanalyzingResults_EcPp3.sh 0 V0 'MaxMetNar' 100 'EcPp3' '2_models 3_models'
...
seed=123
cd smac-output/${domainName}_confFLYCOP_scenario_v${id}_generalized/state-run${seed}
...
param6=`tail log-run${seed}.txt | egrep "p01_sucr1" | awk -F'p01_sucr1' '{print $2}' | cut -d' ' -f12 | sed "s/'//g"`
param7=`tail log-run${seed}.txt | egrep "p01_sucr1" | awk -F'p01_sucr1' '{print $2}' | cut -d' ' -f14 | sed "s/'//g"`
param8=`tail log-run${seed}.txt | egrep "p01_sucr1" | awk -F'p01_sucr1' '{print $2}' | cut -d' ' -f16 | sed "s/'//g"`
param9=`tail log-run${seed}.txt | egrep "p01_sucr1" | awk -F'p01_sucr1' '{print $2}' | cut -d' ' -f18 | sed "s/'//g"`
n_models=$( echo ${param9} | cut -d'_' -f1 )

init_biomass=()
# As many iterations as models in a given consortium architecture
...
echo "Optimal consortium configuration found: " $param1 $param2 $param3 $param4 $param5 $param6 $param7 $param8 $param9
...
python3 -W ignore ../../Scripts/${domainName}_individualTestFLYCOP_v0_generalized.py $param1 $param2 $param3 $param4 $param5 $param6 $param7 $param8 $param9 "$init_biomass" $fitness
...
#rm -r ${domainName}_confFLYCOP_params_v0_generalized.pcs
cp -r configurationResults_consArchitecture.xlsx ..  # This would be the updated version of the configurationsResults table, in xlsx format
cd ../../..
# ---------------------------------------------------------------
