...
# (i.e. uptake rates + 'n_models' SMAC args) should be 'X'
# -----------------------------------------------------------------------------
...
with open(define_conditionals_file, "r") as def_file:
    ...
    for line in lines:
        line = line.strip("\n").split(":")
        architecture_dictionary[int(line[0])] = line[1].split(",")  # List of strain names
        
        
# Base consortium architecture (the simplest one among possibilities)
min_n_models = min(architecture_dictionary.keys())  # Integer
        ...
        for strain in architecture_dictionary[n_models_key]:
            ...
            if strain not in architecture_dictionary[min_n_models]:  
                ...
                with open(smac_pcs_file, "a") as smac_file:
                    smac_file.write("p{0}_biomass_{1} | p{2}_nmodels == {3}_models".format(n_arg_nmodels + n_add_strains, strain, n_arg_nmodels, n_models_key))
