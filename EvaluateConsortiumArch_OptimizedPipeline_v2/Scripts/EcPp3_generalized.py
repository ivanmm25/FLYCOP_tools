...
# biomass loss during more than "n_cycles" for this indicator to detect the given effect
...
# it was detected. Moreover, the current implementation does not either allow for determining 
# if there is just one or more microbes experiencing biomass loss in the community.
# -----------------------------------------------------------------------------

def biomass_evolution_during_simulation(CometsTable, n_cycles = 10, biomass_indexes = []):
    # cycle_count = 0
    ...
    for row in CometsTable.itertuples():  
        ...
        if cycle == 0:  # Initial cycle
            ...
            for index in biomass_indexes:  
                ...
                if float(biomass["biomass"+str(counter)] - last_biomass["biomass"+str(counter)] < 0): 
            ...
            # In case of no biomass loss or equivalent
            if not biomass_track and not biomass_loss_in_cycle and not lower_than_init_biomass: endCycle = cycle
        ...
        # ---------------------------------------------------------------------
        if (count_cons_cycles >= 10 or lower_than_init_biomass) and not biomass_track:
            biomass_track = 1
            ...
            # -----------------------------------------------------------------
            # This 'break' is unnecessary, we can let the function to read the whole
            # COMETS_file even if the biomass loss effect has been already determined
            # to be present. 
            
            # However, once the effect is present, the variables 'biomass_track' 
            # and 'endCycle' are not going to change. The only interest of 
            # letting the function to work until the end of the COMETS_file would 
            # be to determine the whole extension of the effect ('dead_cycles' string)
    ...
    # End line of the simulation
    finalLine = CometsTable.iloc[endCycle].to_list()
        
    if biomass_track:
        dead_cycles = str(initial_dead)+"-"+str(last_dead)  # biomass_track = 1
    else:
...
def SelectConsortiumArchitecture(sucr1, frc2, nh4_Ec, nh4_KT, consortium_arch, initial_biomass,
  ...
  for i in range(repeat):
        ...
        n_metabolites = 7  # 7 metabolites to track (manual adjustment by user). In this case: sucr 2saku fru nar nh4 pi o2
        ...
        subprocess.run(['../../Scripts/plot_biomassX2_vs_4mediaItem_glic_generalized.sh template2 sucr 2saku fru nar nh4 pi o2 '+str(maxCycles)+' '+baseConfig+' blue black darkmagenta yellow orange aquamarine '+strains_string], shell=True)
        ...
        biomass_track, dead_process, initLine, finalLine = biomass_evolution_during_simulation(CometsTable, n_cycles = 10, biomass_indexes = biomass_indexes)
        ...
        ####################################################
        fitNar = tot_MetNar / (total_final_biomass)  # Final metilated naringenin yield over GLOBAL biomass (all microorganisms in the consortium)
        ...
        print("\nFitness (mM/gL) in final cycle: "+str(round(fitness,6)))
