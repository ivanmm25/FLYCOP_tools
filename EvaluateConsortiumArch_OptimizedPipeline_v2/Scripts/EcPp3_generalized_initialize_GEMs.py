...
def initialize_models_iEC1364_W_exc_metilator(sucr1, nh4_Ec, temporal_folder, models_summary):
 # Only to run 1st time, to build the model!
 if not(os.path.exists('ModelsInput/iEC1364_W_unique_saku2.xml')):
  ...
  if not os.path.exists('iEC1364_W_exc_metilator_tmp.mat.txt'):
    ...
    dictOptValuemetnar = cobra.flux_analysis.flux_variability_analysis(model, {'EX_2saku(e)'}, fraction_of_optimum=((1-0.20)))
...
def initialize_models_iEC1364_W_unique_saku2(sucr1, nh4_Ec, temporal_folder, models_summary):
 # Only to run 1st time, to build the model!
 if not(os.path.exists('ModelsInput/iEC1364_W_unique_saku2.xml')):
     print('ERROR! Not iEC1364_W_unique_saku2.xml file with GEM in ModelsInput!')
 else:
  ...
  if not os.path.exists('iEC1364_W_unique_saku2_tmp.mat.txt'):
    ...
    dictOptValueFru = cobra.flux_analysis.flux_variability_analysis(model, {'EX_fru(e)'}, fraction_of_optimum=(1-0.20))
    dictOptValuepCA = cobra.flux_analysis.flux_variability_analysis(model, {'EX_T4hcinnm(e)'}, fraction_of_optimum=((1-0.20)))
    dictOptValueglycnar = cobra.flux_analysis.flux_variability_analysis(model, {'EX_2saku(e)'}, fraction_of_optimum=((1-0.20)))
    ...
    # =======================
    MetNarLimit=dictOptValueglycnar['EX_2saku(e)']['maximum']
    model.reactions.get_by_id("2saku_tpp").bounds=(MetNarLimit, 1000)  
    model.reactions.get_by_id("2saku_tex").bounds=(MetNarLimit, 1000)  
    model.reactions.get_by_id("EX_2saku(e)").bounds=(MetNarLimit, MetNarLimit)  
